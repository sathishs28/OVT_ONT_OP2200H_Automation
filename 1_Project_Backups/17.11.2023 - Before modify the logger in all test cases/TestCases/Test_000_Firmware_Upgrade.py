...
class Test_000_Firmware_Upgrade:
    ...
    def test_firmware_upgrade(self, setup):
        self.logger.info("************* Test_000_Firmware_Upgrade :: test_firmware_upgrade *************")
        try:
            self.driver = setup
            ...
            # print(bf_fw_ver)
            self.logger.info(f"******** Current device running firmware version - {bf_fw_ver} **************")
            self.ap.click_admin_mainmenu()
            self.ap.click_admin_submenu()
            self.ap.click_firmware_upg()
            self.logger.info("******** test_firmware_upgrade Test case is starting ********")
            ...
            if is_file_present(fw_file_path):
                self.logger.info(f"******** test_firmware_upgrade the fw file '{fw_file_path}' is present *******")
                self.logger.info("******** test_firmware_upgrade - New firmware is going to update ********")
                ...
                if self.ap.verify_progress_bar_ele().is_displayed():
                    self.logger.info("******** Firmware upgrading process is starting *******")
                    # self.ap.get_fw_upg_progress_status()
                    i = 0
                    while i <= 99:
                        i = int(self.ap.get_fw_upg_progress_status().rstrip('%'))
                        self.logger.info(f"******** Firmware upgrading - {i}% **************")
                        time.sleep(1)
                    self.logger.info("******** Firmware upgraded - 100% Successfully *******")
                    ...
                    if logo_element.is_displayed():
                        self.logger.info("******* Homepage is loaded *******")
                    else:
                        self.logger.error("******* Homepage is not loaded - Something wrong *******")
                    ...
                    if bf_fw_ver != af_fw_ver:
                        self.logger.info(f"******* Latest updated firmware version - {af_fw_ver} *******")
                        self.logger.info("******** test_firmware_upgrade Test case is completed & Success ********")
                        self.lp.click_logout()
                        self.driver.close()
                        assert True
                    else:
                        self.logger.error(f"******* Latest updated firmware is still same - Check Firmware *******")
                        self.logger.error("******** test_firmware_upgrade Test case is Failed ********")
                    ...
                    self.logger.info("******** Firmware upgrading process is not showing *******")
                    return
            else:
                self.logger.error(f"******* test_firmware_upgrade the fw file '{fw_file_path}' is not present *******")

                self.logger.error("******** test_firmware_upgrade Test case is stopped ********")
            ...
            self.logger.error("******** test_firmware_upgrade Test case is stopped - Refer belo logs *******")
            self.logger.error(f"Page load failed: {str(e1)}")
            self.driver.switch_to.default_content()
            self.lp.click_logout()
            self.driver.close()
            assert False
