...
class Test_003_New_WAN:
    ...
    def test_add_bridge_wan_1(self, setup):
        self.driver = setup
        self.driver.get(self.device_URL)
        try:
            self.logger.info("********************* Test_003_New_WAN :: 1-test_add_bridge_wan *********************")
            self.logger.info("******* test_add_bridge_wan Test case is started & Verifying *******")
            # Login Device
            ...
            if status == self.wp.verify_apply_status():
                ...
                if bf_wan_list != af_wan_list:
                    ...
                    self.logger.info("******* test_add_bridge_wan Test case is completed *******")
                    self.logger.info("******* Newly added WAN Interface Name - " + af_wan_list[new_wan_index].text +
                                     '*******')
                    self.logger.info("******* test_add_bridge_wan Test case is success *******")
                    assert True
                else:
                    # print("Newly added WAN is not there")
                    self.driver.save_screenshot(snap_path + "test_add_bridge_wan_error.png")
                    self.logger.error("************** Newly added WAN is not added **************")
                    self.logger.error("************** test_add_bridge_wan Test case is failed **************")
                    assert False
                self.driver.switch_to.default_content()
                self.lp.click_logout()
                self.driver.close()
                ...
                self.driver.save_screenshot(snap_path + "test_add_bridge_wan_error.png")
                self.logger.error("************** Status -" + self.wp.verify_apply_status() + "**************")
                self.logger.error("************** test_add_bridge_wan Test case is failed **************")
                self.driver.switch_to.default_content()
                self.lp.click_logout()
                self.driver.close()
            ...
            self.logger.warning("************** test_add_bridge_wan Test case is stopped - Webpage is not loaded ("
                                "Refer belo logs**************")
    ...
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_add_ipoe_wan_2(self, setup):
        self.driver = setup
        self.driver.get(self.device_URL)
        try:
            self.logger.info("********************* Test_003_New_WAN :: 2-test_add_ipoe_wan *********************")
            self.logger.info("******* test_add_ipoe_wan Test case is started & Verifying *******")
            # Login Device
            self.lp = LoginPage(self.driver)
            self.lp.login(self.username, self.password)
            ...
            if status == self.wp.verify_apply_status():
                ...
                if bf_wan_list != af_wan_list:
                    new_wan_index = len(af_wan_list) - 2
                    print("Newly added WAN Interface Name - " + af_wan_list[new_wan_index].text)
                    self.logger.info("******* test_add_ipoe_wan Test case is completed *******")
                    self.logger.info("******* Newly added WAN Interface Name - " + af_wan_list[new_wan_index].text +
                                     '*******')
                    self.logger.info("******* test_add_ipoe_wan Test case is success *******")
                    assert True
                else:
                    self.driver.save_screenshot(snap_path + "test_add_ipoe_wan_error.png")
                    self.logger.error("************** Newly added WAN is not added **************")
                    self.logger.error("************** test_add_ipoe_wan Test case is failed **************")
                    assert False
                self.driver.switch_to.default_content()
                self.lp.click_logout()
                self.driver.close()
            ...
            self.logger.warning("************** test_add_ipoe_wan Test case is stopped - Webpage is not loaded ("
                                "Refer belo logs**************")
            self.logger.warning(f"Page load failed: {str(e1)}")
            self.driver.switch_to.default_content()
            self.lp.click_logout()
    ...
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_add_pppoe_wan_3(self, setup):
        self.driver = setup
        self.driver.get(self.device_URL)
        try:
            self.logger.info("********************* Test_003_New_WAN :: 3-test_add_pppoe_wan *********************")
            self.logger.info("******* test_add_pppoe_wan Test case is started & Verifying *******")
            # Login Device
            self.lp = LoginPage(self.driver)
            self.lp.login(self.username, self.password)
            ...
            if status == self.wp.verify_apply_status():
                ...
                if bf_wan_list != af_wan_list:
                    new_wan_index = len(af_wan_list) - 2
                    # print("Newly added WAN Interface Name - " + af_wan_list[new_wan_index].text)
                    self.logger.info("******* test_add_pppoe_wan Test case is completed *******")
                    self.logger.info("******* Newly added WAN Interface Name - " + af_wan_list[new_wan_index].text +
                                     '*******')
                    self.logger.info("******* test_add_pppoe_wan Test case is success *******")
                    assert True
                else:
                    self.driver.save_screenshot(snap_path + "test_add_pppoe_wan_error.png")
                    self.logger.error("************** Newly added WAN-pppoe is not added **************")
                    self.logger.error("************** test_add_pppoe_wan Test case is failed **************")
                    assert False
                self.driver.switch_to.default_content()
                self.lp.click_logout()
                self.driver.close()
            ...
            self.logger.warning("************** test_add_pppoe_wan Test case is stopped - Webpage is not loaded, "
                                "Refer belo logs**************")
            self.logger.warning(f"Page load failed: {str(e1)}")
            self.driver.switch_to.default_content()
            self.lp.click_logout()
    ...
    def test_workout(self, setup):
        ...
        """
        self.logger.info("Testing Sample log generated")
        self.driver.switch_to.default_content()
        self.lp.click_logout()
        self.driver.close()