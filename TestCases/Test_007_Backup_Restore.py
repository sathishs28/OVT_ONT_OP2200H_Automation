...
class Test_007_Backup_Restore:
    ...
    """ Below methods are testing purposes """


"""
    @pytest.fixture
    def test_1_result(self, request):
        yield
        self.__class__.test_1_failed = request.node.session.testsfailed > 0

    @pytest.mark.Testing2
    def test_1(self, test_1_result):
        if not get_list_files():
            print("Backup directory is empty")
            pytest.skip("Backup directory is empty")
        ...
        if get_latest_file.endswith(".xml"):
            ...
            self.__class__.test_1_failed = True
            print("\n******* Latest backup configuration file is not there...! *******")
            assert False

    @pytest.mark.Testing2
    # @pytest.mark.skipif(lambda: Test_007_Backup_Restore.test_1_failed, reason="Skipping if test_1 failed")
    def test_2(self):
        print(f"Test 1 failed: {self.__class__.test_1_failed}")
        print(f"Test 1 Failed print variable: {self.test_1_failed}")
        if self.__class__.test_1_failed:
            pytest.skip("Skipping test_2 because test_1 failed")
